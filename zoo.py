...
class Zoo:
    ...
    def cleaning(self):
        ...
        for enclosure in self.all_Enclosures:
            if len(enclosure.cleaning_records) > 0:
                ...
                returning_object[
                    enclosure.name
                ] = f"Month:{month_more} Day:{futureday} Responsible person {self.caretakers[person].name}"
        return returning_object
    ...
    def feeding(self):
        ...
        for animal in self.animals:
            ...
            returning_object[
                animal.animal_id
            ] = f"Month:{month_more} Day:{futureday} Responsible person {animal.care_taker}"
        return returning_object
    ...
    def stat(self):
        ...
        average_animals_in_enclosure = sum(all_animals_in_enclosures) / len(
            self.all_Enclosures
        )
        available_space = {}
        for x in self.all_Enclosures:
